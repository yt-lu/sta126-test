...
server <- function(input, output){
  ...
  output$textblocknum <- renderText({
    ...
    if(!is.na(input$id) & (input$id %% 7 == 6 | input$id %% 7 == 5 | input$id %% 7 ==  4)){
    out <- paste(out, 
          "<li>Test if the true average price of the regular gas is $1.671 </li>"
          )
    }
    
    if(!is.na(input$id) & (input$id %% 7 == 0 | input$id %% 7 == 1 | input$id %% 7 ==  2 | input$id %% 7 == 3)){
      out <- paste(out, 
  ...
  output$textblockcat <- renderText({
    ...
    if(!is.na(input$id) & (input$id %% 7 == 0 | input$id %% 7 == 1 | input$id %% 7 ==  2 | input$id %% 7 == 3)){
      out <- paste(out, 
                   "<li>Test if the true proportion of people who are more concerned that relaxing stay-at-home restrictions would lead to more COVID-19 deaths is 0.62</li>"
      )
    }
